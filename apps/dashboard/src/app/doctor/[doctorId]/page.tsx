...
function PatientCard({
  ...
  return (
    <div
      className={`bg-surface-card border rounded-xl p-4 shadow-sm ${borderClass}`}>
      {/* Header row */}
        ...
        <div className="shrink-0 text-right">
          ...
          {isInProgress && patient.elapsed_minutes !== null && (
            <p
              ...
              {patient.elapsed_minutes} /{" "}
              {patient.expected_consultation_minutes ?? "?"} min
              {isOvertime && " ⚠"}
            </p>
          )}
        </div>
...
export default function DoctorDashboardPage({
  ...
  const [hasTriggeredOvertime, setHasTriggeredOvertime] = useState<
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        currentPatient.elapsed_minutes >=
        currentPatient.expected_consultation_minutes
      ) {
        if (!hasTriggeredOvertime[currentPatient.visit_id]) {
          const patientsWaiting = patients.filter(
            (p) => p.step_status === "pending",
          ).length;
          ...
          setHasTriggeredOvertime((prev) => ({
            ...prev,
            [currentPatient.visit_id]: true,
          }));
        }
      }
    }
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-content-primary mt-0.5">
                <span className="font-medium">
                  {arrivingToast.patient_name}
                </span>
                {" viene de "}
                <span className="font-medium text-content-secondary">
                  {arrivingToast.from_area}
                </span>
